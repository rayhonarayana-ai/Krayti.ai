...
async function runIdempotencyTests() {
  ...
  try {
    ...
    assert(firstObs.idempotencyKey === `obs_ex_${studentId}_${submissionId}`, 'idempotencyKey is derived deterministically from submissionId');
    ...
    assert(recordedObservations[1].idempotencyKey === `obs_ex_${studentId}_${newSubmissionId}`, 'New submission has distinct idempotencyKey');
    ...
    assert(obsCalc.idempotencyKey === `obs_ex_${studentId}_sub-calc-789`, 'Business idempotencyKey remained perfectly stable despite different event IDs');
